void transmittance_giessen_ipn_20180108(){
  ...
  /*   if(i == 11 || i == 15 || i == 23 || i == 31 || i == 33 || i == 34 || i == 37 || i == 38 || i == 43 || i == 44){ */
      ...
      string filler_Tanja = "/sps/clas/hosanko/NPS/PbWO4/CUA/";
      string filler2 = ".csv";
      ...
      string file_before_Tanja = filler_Tanja + filename_before_Tanja + filler2;
      ...
      double wavelength_before_Tanja[n] = {0.}; 
	...
	myfile_giessen>>dummy>>dummy;    
      ...
      myfile_before_Tanja>>dummy>>origin[0]>>origin[1];
      myfile_before_Tanja>>dummy>>xmax[0]>>xmax[1];
      myfile_before_Tanja>>dummy>>ymax[0]>>ymax[1];


      double xscale = (xmax[0] - origin[0])/600.;
      double yscale = (ymax[1] - origin[1])/100.;
	...
	/* if( wavelength_before[j] == 620. || wavelength_before[j] == 420. || wavelength_before[j] == 360.)	myfile_before_Tanja>>wavelength_before[j]>>transmittance_before_Tanja[j]; */
      ...
      for (j = 0 ; j < n ; j++){
	myfile_before_Tanja>>dummy>>wavelength_before_Tanja[j]>>transmittance_before_Tanja[j];
	/* cout<<j<<",\t"<<wavelength_before_Tanja[j]<<",\t"<<transmittance_before_Tanja[j]<<endl; */
	wavelength_before_Tanja[j] = (wavelength_before_Tanja[j] - origin[0])/xscale + 200.;
	transmittance_before_Tanja[j] = (transmittance_before_Tanja[j] - origin[1])/yscale;
	cout<<j<<",\t"<<wavelength_before_Tanja[j]<<",\t"<<transmittance_before_Tanja[j]<<endl;
      }
	...
	//	cout<<wavelength_giessen[j]<<endl;
      ...
      TGraph *gr_before_Tanja = new TGraph(n,wavelength_before_Tanja,transmittance_before_Tanja);
      ...
      string dir = "output/20180108/";
      string png = "_transmittance_giessen_ipn_tanja_comparison_2018018.png";
